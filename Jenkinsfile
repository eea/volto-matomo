pipeline {
  ...
  environment {
        GIT_NAME = "volto-matomo"
        NAMESPACE = "@eeacms"
        SONARQUBE_TAGS = "volto.eea.europa.eu"
        DEPENDENCIES = "@datapunt/matomo-tracker-react"
    }
    ...
    stage('Code') {
      steps {
        parallel(
          ...
          "Style lint": {
            node(label: 'docker') {
              sh '''docker pull plone/volto-addon-ci:11.1.0; docker run -i --rm --name="$BUILD_TAG-stylelint" -e NAMESPACE="$NAMESPACE" -e DEPENDENCIES="$DEPENDENCIES" -e GIT_NAME=$GIT_NAME -e GIT_BRANCH="$BRANCH_NAME" -e GIT_CHANGE_ID="$CHANGE_ID" plone/volto-addon-ci:11.1.0 stylelint'''
            }
          },

          "Prettier": {
            node(label: 'docker') {
              sh '''docker pull plone/volto-addon-ci:11.1.0; docker run -i --rm --name="$BUILD_TAG-prettier" -e NAMESPACE="$NAMESPACE" -e DEPENDENCIES="$DEPENDENCIES" -e GIT_NAME=$GIT_NAME -e GIT_BRANCH="$BRANCH_NAME" -e GIT_CHANGE_ID="$CHANGE_ID" plone/volto-addon-ci:11.1.0 prettier'''
            }
          }
        )
      }
        ...
        parallel(
          ...
          "Volto": {
            node(label: 'docker') {
              script {
                try {
                  sh '''docker pull plone/volto-addon-ci:11.1.0'''
                  sh '''docker run -i --rm --name="$BUILD_TAG-prettier" -e NAMESPACE="$NAMESPACE" -e DEPENDENCIES="$DEPENDENCIES" -e GIT_NAME=$GIT_NAME -e GIT_BRANCH="$BRANCH_NAME" -e GIT_CHANGE_ID="$CHANGE_ID" plone/volto-addon-ci:11.1.0 test'''
        ...
        parallel(
          ...
          "Cypress": {
            node(label: 'docker') {
              script {
                try {
                  sh '''docker pull plone; docker run -d --name="$BUILD_TAG-plone" -e SITE="Plone" -e ADDONS="eea.geolocation eea.zotero" -e PROFILES="profile-plone.restapi:blocks" plone fg'''
